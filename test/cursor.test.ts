  ...
  describe("sort", () => {
    describe("single field ascending", () => {
      ...
      it("should sort dates ascending", async () => {
        ...
        assert.strictEqual(docs.length, 3);
        assert.strictEqual(
          (docs[0].createdAt as Date).getTime(),
          date1.getTime()
        );
        assert.strictEqual(
          (docs[1].createdAt as Date).getTime(),
          date2.getTime()
        );
        assert.strictEqual(
          (docs[2].createdAt as Date).getTime(),
          date3.getTime()
        );
      });
    });
  ...
  describe("projection", () => {
    ...
    describe("field exclusion", () => {
      it("should exclude specified fields", async () => {
        ...
        const docs = await collection
          .find({}, { projection: { email: 0, city: 0 } })
          .toArray();
    ...
    describe("projection with nested fields", () => {
      it("should project nested fields", async () => {
        ...
        assert.strictEqual(
          (docs[0].address as { zip?: string }).zip,
          undefined
        );
      });
    });
