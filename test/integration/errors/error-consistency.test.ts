...
describe(`Error Consistency (${getTestModeName()})`, () => {
  ...
  describe('Expression operator type errors', () => {
    it('should error on $abs with non-numeric value', async () => {
      ...
      await assert.rejects(
        async () => {
          await collection.aggregate([{ $project: { result: { $abs: '$value' } } }]).toArray();
        },
        (err: Error) => {
          assert.ok(
            err.message.includes('numeric') || err.message.includes('number'),
    ...
    it('should error on $sqrt with negative number', async () => {
      ...
      await assert.rejects(
        async () => {
          await collection.aggregate([{ $project: { result: { $sqrt: '$value' } } }]).toArray();
        },
        (err: Error) => {
          assert.ok(
            err.message.includes('negative') || err.message.includes('sqrt'),
    ...
    it('should error on $log with non-positive number', async () => {
      ...
      await assert.rejects(
        async () => {
          await collection.aggregate([{ $project: { result: { $ln: '$value' } } }]).toArray();
        },
        (err: Error) => {
          assert.ok(
    ...
    it('should error on $dateFromString with invalid date', async () => {
      ...
      await assert.rejects(
        async () => {
          await collection
            .aggregate([{ $project: { result: { $dateFromString: { dateString: '$dateStr' } } } }])
            .toArray();
        },
        (err: Error) => {
          assert.ok(
    ...
    it('should error on $size with non-array', async () => {
      ...
      await assert.rejects(
        async () => {
          await collection.aggregate([{ $project: { result: { $size: '$value' } } }]).toArray();
        },
        (err: Error) => {
          assert.ok(
            err.message.includes('array'),
  ...
  describe('Query operator errors', () => {
    it('should error on $all with non-array argument', async () => {
      ...
      await assert.rejects(
        ...
        (err: Error) => {
          assert.ok(err.message.includes('array'), `Expected array error, got: ${err.message}`);
          return true;
        }
      );
    });
  ...
  describe('Aggregation stage errors', () => {
    it('should error on unknown aggregation stage', async () => {
      ...
      await assert.rejects(
        async () => {
          await collection.aggregate([{ $unknownStage: {} }]).toArray();
        },
        (err: Error) => {
          assert.ok(
