...
describe(`Advanced Aggregation Pipeline Tests (${getTestModeName()})`, () => {
  ...
  describe('Expression Operators', () => {
    describe('$add', () => {
      it('should add numbers', async () => {
        ...
        const results = await collection
          .aggregate([{ $project: { sum: { $add: ['$a', '$b'] }, _id: 0 } }])
          .toArray();
      ...
      it('should add multiple numbers', async () => {
        ...
        const results = await collection
          .aggregate([{ $project: { sum: { $add: ['$a', '$b', '$c', '$d'] }, _id: 0 } }])
          .toArray();
    ...
    describe('$subtract', () => {
      it('should subtract numbers', async () => {
        ...
        const results = await collection
          .aggregate([{ $project: { diff: { $subtract: ['$a', '$b'] }, _id: 0 } }])
          .toArray();
    ...
    describe('$multiply', () => {
      it('should multiply numbers', async () => {
        ...
        const results = await collection
          .aggregate([{ $project: { product: { $multiply: ['$a', '$b'] }, _id: 0 } }])
          .toArray();
      ...
      it('should multiply multiple numbers', async () => {
        ...
        const results = await collection
          .aggregate([{ $project: { product: { $multiply: ['$a', '$b', '$c'] }, _id: 0 } }])
          .toArray();
    ...
    describe('$divide', () => {
      it('should divide numbers', async () => {
        ...
        const results = await collection
          .aggregate([{ $project: { quotient: { $divide: ['$a', '$b'] }, _id: 0 } }])
          .toArray();
    ...
    describe('$concat', () => {
      it('should concatenate strings', async () => {
        ...
        const results = await collection
          .aggregate([{ $project: { result: { $concat: ['$first', ' ', '$second'] }, _id: 0 } }])
          .toArray();
    ...
    describe('$toUpper / $toLower', () => {
      it('should convert string to uppercase', async () => {
        ...
        const results = await collection
          .aggregate([{ $project: { upper: { $toUpper: '$text' }, _id: 0 } }])
          .toArray();
      ...
      it('should convert string to lowercase', async () => {
        ...
        const results = await collection
          .aggregate([{ $project: { lower: { $toLower: '$text' }, _id: 0 } }])
          .toArray();
    ...
    describe('$ifNull', () => {
      it('should return first non-null value', async () => {
        ...
        const results = await collection
          .aggregate([{ $project: { result: { $ifNull: ['$a', '$b'] }, _id: 0 } }])
          .toArray();
    ...
    describe('$size (expression)', () => {
      it('should return array length', async () => {
        ...
        const results = await collection
          .aggregate([{ $project: { count: { $size: '$items' }, _id: 0 } }])
          .toArray();
  ...
  describe('$group Stage', () => {
    describe('Basic Grouping', () => {
      ...
      it('should group by compound _id', async () => {
        ...
        assert.strictEqual(results.length, 2);
        const jan = results.find((r) => (r._id as { month: number }).month === 1);
  ...
  describe('$addFields / $set Stage', () => {
    it('should add new field with literal value', async () => {
      ...
      const results = await collection.aggregate([{ $addFields: { status: 'active' } }]).toArray();
    ...
    it('should preserve existing fields', async () => {
      ...
      const results = await collection.aggregate([{ $addFields: { d: 4 } }]).toArray();
    ...
    it('$set should behave identically to $addFields', async () => {
      ...
      const results = await collection.aggregate([{ $set: { status: 'active' } }]).toArray();
  ...
  describe('$replaceRoot Stage', () => {
    ...
    it('should throw for null newRoot value', async () => {
      ...
      await assert.rejects(
        async () => {
          await collection.aggregate([{ $replaceRoot: { newRoot: '$address' } }]).toArray();
        },
        (err: Error) => {
  ...
  describe('$lookup Stage', () => {
    ...
    it('should support pipeline form with let and pipeline', async () => {
      ...
      assert.strictEqual((results[0].fromItems as Document[])[0].description, 'product 1');
    ...
    it('should filter and aggregate in pipeline lookup', async () => {
      ...
      assert.strictEqual((results[0].qualifyingTransactions as Document[]).length, 1);
      // Bob: minSpend 50, so transactions >= 50 qualify (200)
      assert.strictEqual((results[1].qualifyingTransactions as Document[]).length, 1);
    });
    ...
    it('should return all foreign docs when pipeline has no $match', async () => {
      ...
      await items.insertMany([{ value: 10 }, { value: 20 }, { value: 30 }]);
  ...
  describe('$out Stage', () => {
    it('should write results to collection', async () => {
      ...
      await source.aggregate([{ $match: { status: 'active' } }, { $out: 'out_target' }]).toArray();
    ...
    it('should replace existing collection', async () => {
      ...
      // Pre-populate target
      await target.insertMany([{ old: 'data1' }, { old: 'data2' }]);

      // Source data
      await source.insertMany([{ new: 'data1' }]);

      await source.aggregate([{ $out: 'out_target2' }]).toArray();
    ...
    it('should return empty array', async () => {
      ...
      const results = await source.aggregate([{ $out: 'out_target3' }]).toArray();
  ...
  describe('$merge Stage', () => {
    ...
    it('should keep existing documents (whenMatched: keepExisting)', async () => {
      ...
      // Pre-populate target
      await target.insertMany([{ _id: 1, name: 'Original Alice', score: 50 }]);
    ...
    it('should merge fields (whenMatched: merge)', async () => {
      ...
      await target.insertMany([{ _id: 1, name: 'Alice', extra: 'preserved' }]);

      // Source with additional/updated data
      await source.insertMany([{ _id: 1, score: 100, status: 'active' }]);
    ...
    it('should discard non-matching documents (whenNotMatched: discard)', async () => {
      ...
      // Pre-populate target
      await target.insertMany([{ _id: 1, name: 'Alice' }]);
    ...
    it('should match on custom field (on option)', async () => {
      ...
      await target.insertMany([{ _id: 100, uniqueId: 'user1', name: 'Old Name' }]);
      ...
      const targetDocs = await target.find({}).toArray();
      targetDocs.sort((a, b) => String(a.uniqueId).localeCompare(String(b.uniqueId)));
