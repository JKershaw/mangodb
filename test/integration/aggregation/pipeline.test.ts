...
describe(`Aggregation Pipeline Tests (${getTestModeName()})`, () => {
    ...
    it('should throw for unknown stage', async () => {
      ...
      await assert.rejects(
        ...
        (err: Error) => {
          ...
          assert.ok(err.message.includes('$unknownStage') || err.message.includes('unknownStage'));
          return true;
        }
      );
    });
  ...
  describe('$match Stage', () => {
    ...
    it('should support comparison operators', async () => {
      ...
      await collection.insertMany([{ age: 15 }, { age: 20 }, { age: 25 }, { age: 30 }]);
    ...
    it('should support array operators', async () => {
      ...
      const results = await collection.aggregate([{ $match: { tags: 'js' } }]).toArray();
    ...
    it('should support dot notation for nested fields', async () => {
      ...
      const results = await collection.aggregate([{ $match: { 'user.name': 'Alice' } }]).toArray();
  ...
  describe('$project Stage', () => {
    describe('Inclusion Mode', () => {
      it('should include specified fields only', async () => {
        ...
        const results = await collection.aggregate([{ $project: { name: 1, age: 1 } }]).toArray();
      ...
      it('should include _id by default', async () => {
        ...
        const results = await collection.aggregate([{ $project: { name: 1 } }]).toArray();
      ...
      it('should allow excluding _id with { _id: 0 }', async () => {
        ...
        const results = await collection.aggregate([{ $project: { name: 1, _id: 0 } }]).toArray();
      ...
      it('should handle nested fields with dot notation', async () => {
        ...
        assert.strictEqual((results[0].user as Record<string, unknown>).name, 'Alice');
        assert.strictEqual((results[0].user as Record<string, unknown>).email, undefined);
      });
    ...
    describe('Exclusion Mode', () => {
      it('should exclude specified fields', async () => {
        ...
        const results = await collection.aggregate([{ $project: { password: 0 } }]).toArray();
      ...
      it('should include all other fields', async () => {
        ...
        const results = await collection.aggregate([{ $project: { c: 0 } }]).toArray();
      ...
      it('should handle { _id: 0 } alone as exclusion mode', async () => {
        ...
        const results = await collection.aggregate([{ $project: { _id: 0 } }]).toArray();
    ...
    describe('Error Cases', () => {
      it('should throw when mixing inclusion and exclusion', async () => {
        ...
        await assert.rejects(
          async () => {
            await collection.aggregate([{ $project: { a: 1, b: 0 } }]).toArray();
          },
          (err: Error) => {
  ...
  describe('$sort Stage', () => {
    ...
    it('should sort by string field (lexicographic)', async () => {
      ...
      const results = await collection.aggregate([{ $sort: { name: 1 } }]).toArray();
    ...
    it('should sort by date field', async () => {
      ...
      const results = await collection.aggregate([{ $sort: { createdAt: 1 } }]).toArray();

      assert.strictEqual((results[0].createdAt as Date).getTime(), date1.getTime());
      assert.strictEqual((results[2].createdAt as Date).getTime(), date3.getTime());
    });
    ...
    it('should handle null/missing values in sort', async () => {
      ...
      await collection.insertMany([{ value: 3 }, { value: null }, { value: 1 }, { noValue: true }]);

      const results = await collection.aggregate([{ $sort: { value: 1 } }]).toArray();

      // Null/missing values should come first in ascending order
      assert.strictEqual(results.length, 4);
      assert.ok(results[0].value === null || results[0].value === undefined);
    });
  });
  ...
  describe('$skip Stage', () => {
    it('should skip first n documents', async () => {
      ...
      const results = await collection.aggregate([{ $sort: { value: 1 } }, { $skip: 2 }]).toArray();
  ...
  describe('$unwind Stage', () => {
    describe('Basic Unwind', () => {
      it('should unwind array into multiple documents', async () => {
        ...
        const results = await collection.aggregate([{ $unwind: '$tags' }]).toArray();
      ...
      it('should handle single-element arrays', async () => {
        ...
        const results = await collection.aggregate([{ $unwind: '$items' }]).toArray();
      ...
      it('should treat non-array value as single-element array', async () => {
        ...
        const results = await collection.aggregate([{ $unwind: '$value' }]).toArray();
      ...
      it('should handle nested array fields with dot notation', async () => {
        ...
        const results = await collection.aggregate([{ $unwind: '$data.items' }]).toArray();
    ...
    describe('includeArrayIndex Option', () => {
      it('should add index field to each unwound document', async () => {
        ...
        const results = await collection
          .aggregate([{ $unwind: { path: '$tags', includeArrayIndex: 'idx' } }])
          .toArray();
      ...
      it('should start index at 0', async () => {
        ...
        const results = await collection
          .aggregate([{ $unwind: { path: '$items', includeArrayIndex: 'position' } }])
          .toArray();
  ...
  describe('Combined Pipeline Tests', () => {
    it('should execute $match -> $sort -> $limit', async () => {
      ...
      const results = await collection
        .aggregate([{ $match: { status: 'active' } }, { $sort: { score: -1 } }, { $limit: 2 }])
        .toArray();
    ...
    it('should execute $match -> $project', async () => {
      ...
      const results = await collection
        .aggregate([{ $match: { status: 'active' } }, { $project: { name: 1, email: 1, _id: 0 } }])
        .toArray();
    ...
    it('should preserve document order through stages', async () => {
      ...
      const results = await collection
        .aggregate([{ $sort: { order: 1 } }, { $project: { order: 1, _id: 0 } }])
        .toArray();
