...
describe(`Object Operators (${getTestModeName()})`, () => {
  ...
  describe("$getField", () => {
    it("should get a field value using short form", async () => {
      ...
      const result = await collection
        .aggregate([
          { $project: { nameValue: { $getField: "name" } } }
        ])
        .toArray();
    ...
    it("should return null for missing field", async () => {
      ...
      const result = await collection
        .aggregate([
          { $project: { missingValue: { $getField: "nonexistent" } } }
        ])
        .toArray();
  ...
  describe("$mergeObjects", () => {
    ...
    it("should work in $group stage", async () => {
      ...
      const categoryA = result.find(r => r._id === "A") as { combinedMeta: { color?: string; size?: string } } | undefined;
      const categoryB = result.find(r => r._id === "B") as { combinedMeta: { color: string } } | undefined;

      // Order of merging depends on document order
      assert.ok(categoryA?.combinedMeta.color === "red" || categoryA?.combinedMeta.size === "large");
