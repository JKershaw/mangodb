...
describe(`Text Search (${getTestModeName()})`, () => {
  ...
  describe('$text query operator', () => {
    it('should find documents containing search term', async () => {
      ...
      const results = await collection.find({ $text: { $search: 'coffee' } }).toArray();
    ...
    it('should find documents matching any word (OR)', async () => {
      ...
      const results = await collection.find({ $text: { $search: 'fox dog' } }).toArray();
    ...
    it('should be case insensitive', async () => {
      ...
      const results = await collection.find({ $text: { $search: 'text' } }).toArray();
    ...
    it('should support phrase search with quotes', async () => {
      ...
      const results = await collection.find({ $text: { $search: '"new york"' } }).toArray();
    ...
    it('should support negation with minus', async () => {
      ...
      const results = await collection.find({ $text: { $search: 'coffee -milk' } }).toArray();
  ...
  describe('$meta projection', () => {
    ...
    it('should allow sorting by text score', async () => {
      ...
      const results = await collection
        .find({ $text: { $search: 'coffee' } }, { projection: { score: { $meta: 'textScore' } } })
  ...
  describe('edge cases', () => {
    it('should return empty for empty search string', async () => {
      ...
      const results = await collection.find({ $text: { $search: '' } }).toArray();
