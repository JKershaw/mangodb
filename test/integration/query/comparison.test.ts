...
describe(`Basic Query Tests (${getTestModeName()})`, () => {
  ...
  describe('Equality matching', () => {
    it('should match string values', async () => {
      ...
      await collection.insertMany([{ name: 'Alice' }, { name: 'Bob' }, { name: 'Alice' }]);
    ...
    it('should match boolean values', async () => {
      ...
      await collection.insertMany([{ active: true }, { active: false }, { active: true }]);
  ...
  describe('Dot notation', () => {
    ...
    it('should access deeply nested fields', async () => {
      ...
      await collection.insertMany([{ a: { b: { c: { d: 1 } } } }, { a: { b: { c: { d: 2 } } } }]);
  ...
  describe('$gt operator', () => {
    it('should match values greater than', async () => {
      ...
      await collection.insertMany([{ value: 5 }, { value: 10 }, { value: 15 }, { value: 20 }]);
    ...
    it('should work with strings (lexicographic)', async () => {
      ...
      await collection.insertMany([{ name: 'apple' }, { name: 'banana' }, { name: 'cherry' }]);
    ...
    it('should work with dates', async () => {
      ...
      await collection.insertMany([{ date: date1 }, { date: date2 }, { date: date3 }]);

      const docs = await collection.find({ date: { $gt: new Date('2024-06-01') } }).toArray();
  ...
  describe('Comparison operator combinations', () => {
    it('should support range queries with $gt and $lt', async () => {
      ...
      const docs = await collection.find({ value: { $gt: 5, $lt: 20 } }).toArray();
    ...
    it('should support inclusive range with $gte and $lte', async () => {
      ...
      const docs = await collection.find({ value: { $gte: 10, $lte: 20 } }).toArray();
  ...
  describe('$in operator', () => {
    it('should match values in array', async () => {
      ...
      const docs = await collection.find({ color: { $in: ['red', 'blue'] } }).toArray();
    ...
    it('should match numbers in array', async () => {
      ...
      const docs = await collection.find({ value: { $in: [2, 4] } }).toArray();
    ...
    it('should return empty when no values match', async () => {
      ...
      const docs = await collection.find({ value: { $in: [10, 20, 30] } }).toArray();
    ...
    it('should match when document field is array and contains any $in value', async () => {
      ...
      const docs = await collection.find({ tags: { $in: ['a', 'd'] } }).toArray();
  ...
  describe('$nin operator', () => {
    it('should match values not in array', async () => {
      ...
      const docs = await collection.find({ color: { $nin: ['red', 'blue'] } }).toArray();
  ...
  describe('Mixed operators with dot notation', () => {
    it('should apply operators to nested fields', async () => {
      ...
      const docs = await collection.find({ 'user.score': { $gte: 75 } }).toArray();
  ...
  describe('Array element dot notation (querying into arrays)', () => {
    ...
    it('should match deeply nested field in array elements', async () => {
      ...
      const docs = await collection.find({ 'orders.product.sku': 'A1' }).toArray();
    ...
    it('should work with comparison operators on array element fields', async () => {
      ...
      const docs = await collection.find({ 'scores.value': { $gte: 80 } }).toArray();
    ...
    it('should handle multiple levels of arrays', async () => {
      ...
      const docs = await collection.find({ 'groups.members.name': 'Alice' }).toArray();
    ...
    it('should not match when no array element has the nested field value', async () => {
      ...
      await collection.insertMany([{ items: [{ name: 'Alice' }, { name: 'Bob' }] }]);
  ...
  describe('Boolean comparison operators', () => {
    it('should compare booleans with $gt (false < true)', async () => {
      ...
      await collection.insertMany([{ active: false }, { active: true }, { active: false }]);

      // In MongoDB, false < true, so $gt: false should match true
      const docs = await collection.find({ active: { $gt: false } }).toArray();
    ...
    it('should compare booleans with $lt (false < true)', async () => {
      ...
      await collection.insertMany([{ active: false }, { active: true }, { active: true }]);

      // $lt: true should match false
      const docs = await collection.find({ active: { $lt: true } }).toArray();
    ...
    it('should compare booleans with $gte', async () => {
      ...
      const docs = await collection.find({ active: { $gte: true } }).toArray();
    ...
    it('should compare booleans with $lte', async () => {
      ...
      const docs = await collection.find({ active: { $lte: false } }).toArray();
  ...
  describe('Edge cases', () => {
    it('should handle negative array indices (no match)', async () => {
      ...
      await collection.insertMany([{ items: ['a', 'b', 'c'] }]);
