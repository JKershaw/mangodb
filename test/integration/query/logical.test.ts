...
describe(`Logical Operator Tests (${getTestModeName()})`, () => {
  ...
  describe('$exists operator', () => {
    ...
    it('should consider null values as existing', async () => {
      ...
      const docs = await collection.find({ value: { $exists: true } }).toArray();
    ...
    it('should work with dot notation for nested fields', async () => {
      ...
      const docs = await collection.find({ 'user.email': { $exists: true } }).toArray();
    ...
    it('should handle deeply nested field existence', async () => {
      ...
      const docs = await collection.find({ 'a.b.c': { $exists: true } }).toArray();
    ...
    it('should combine $exists with other operators', async () => {
      ...
      const docs = await collection.find({ score: { $exists: true, $gte: 80 } }).toArray();
    ...
    it('should work with array element dot notation for $exists: true', async () => {
      ...
      const docs = await collection.find({ 'items.name': { $exists: true } }).toArray();
    ...
    it('should work with array element dot notation for $exists: false', async () => {
      ...
      const docs = await collection.find({ 'items.name': { $exists: false } }).toArray();
  ...
  describe('$and operator', () => {
    it('should match documents satisfying all conditions', async () => {
      ...
      const docs = await collection.find({ $and: [{ age: 30 }, { active: true }] }).toArray();
    ...
    it('should work with comparison operators in conditions', async () => {
      ...
      await collection.insertMany([{ value: 5 }, { value: 15 }, { value: 25 }, { value: 35 }]);
    ...
    it('should allow same field with different operators', async () => {
      ...
      await collection.insertMany([{ score: 50 }, { score: 75 }, { score: 100 }]);
    ...
    it('should combine with other field conditions', async () => {
      ...
      const docs = await collection.find({ type: 'A', $and: [{ value: { $gte: 10 } }] }).toArray();
    ...
    it('should throw error for empty $and array', async () => {
      ...
      await assert.rejects(async () => await collection.find({ $and: [] } as never).toArray(), {
  ...
  describe('$or operator', () => {
    ...
    it('should work with comparison operators', async () => {
      ...
      await collection.insertMany([{ value: 5 }, { value: 50 }, { value: 150 }]);
    ...
    it('should throw error for empty $or array', async () => {
      ...
      await assert.rejects(async () => await collection.find({ $or: [] } as never).toArray(), {
  ...
  describe('$not operator', () => {
    it('should invert comparison operator result', async () => {
      ...
      await collection.insertMany([{ value: 10 }, { value: 30 }, { value: 50 }]);

      // $not: { $gt: 25 } should match values <= 25
      const docs = await collection.find({ value: { $not: { $gt: 25 } } }).toArray();
    ...
    it('should work with $lt operator', async () => {
      ...
      await collection.insertMany([{ age: 15 }, { age: 25 }, { age: 35 }]);

      // $not: { $lt: 20 } should match values >= 20
      const docs = await collection.find({ age: { $not: { $lt: 20 } } }).toArray();
    ...
    it('should work with $eq operator', async () => {
      ...
      await collection.insertMany([{ color: 'red' }, { color: 'blue' }, { color: 'green' }]);

      const docs = await collection.find({ color: { $not: { $eq: 'red' } } }).toArray();
  ...
  describe('$nor operator', () => {
    ...
    it('should throw error for empty $nor array', async () => {
      ...
      await assert.rejects(async () => await collection.find({ $nor: [] } as never).toArray(), {
  ...
  describe('Error handling', () => {
    it('should throw error when $and is not an array', async () => {
      ...
      await assert.rejects(
        async () => await collection.find({ $and: 'not an array' } as never).toArray(),
    ...
    it('should throw error when $or is not an array', async () => {
      ...
      await assert.rejects(
        async () => await collection.find({ $or: { a: 1 } } as never).toArray(),
    ...
    it('should throw error when $nor is not an array', async () => {
      ...
      await assert.rejects(async () => await collection.find({ $nor: null } as never).toArray(), {
    ...
    it('should throw error when $and is used at field level', async () => {
      ...
      await assert.rejects(
        async () => await collection.find({ field: { $and: [{ a: 1 }] } } as never).toArray(),
    ...
    it('should throw error when $or is used at field level', async () => {
      ...
      await assert.rejects(
        async () => await collection.find({ field: { $or: [{ a: 1 }] } } as never).toArray(),
    ...
    it('should throw error when $nor is used at field level', async () => {
      ...
      await assert.rejects(
        async () => await collection.find({ field: { $nor: [{ a: 1 }] } } as never).toArray(),
    ...
    it('should throw error when $not wraps a non-operator value', async () => {
      ...
      await assert.rejects(
        async () => await collection.find({ field: { $not: 'value' } } as never).toArray(),
