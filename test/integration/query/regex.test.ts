...
describe(`Regular Expression Tests (${getTestModeName()})`, () => {
  ...
  describe('$regex Query Operator', () => {
    describe('Basic Pattern Matching', () => {
      it('should match start of string with ^', async () => {
        ...
        await collection.insertMany([{ name: 'Alice' }, { name: 'Bob' }, { name: 'Anna' }]);
      ...
      it('should match end of string with $', async () => {
        ...
        const docs = await collection.find({ email: { $regex: '@gmail\\.com$' } }).toArray();
      ...
      it('should match pattern anywhere in string', async () => {
        ...
        const docs = await collection.find({ description: { $regex: 'urgent' } }).toArray();
      ...
      it('should match with special regex characters escaped', async () => {
        ...
        const docs = await collection.find({ path: { $regex: 'foo\\.bar' } }).toArray();
      ...
      it('should not match non-string fields', async () => {
        ...
        await collection.insertMany([{ value: 25 }, { value: '25' }, { value: { num: 25 } }]);

        const docs = await collection.find({ value: { $regex: '25' } }).toArray();
      ...
      it('should not match null values', async () => {
        ...
        await collection.insertMany([{ name: null }, { name: 'Alice' }, { name: 'Bob' }]);
      ...
      it('should not match undefined/missing fields', async () => {
        ...
        await collection.insertMany([{ name: 'Alice' }, { other: 'field' }, { name: 'Bob' }]);
      ...
      it('should match empty pattern against any string', async () => {
        ...
        await collection.insertMany([{ name: 'Alice' }, { name: '' }, { name: 'Bob' }]);
    ...
    describe('Options', () => {
      ...
      it('should support multiline (m) option', async () => {
        ...
        const docsNoMultiline = await collection.find({ text: { $regex: '^world' } }).toArray();
      ...
      it('should support dotAll (s) option', async () => {
        ...
        await collection.insertMany([{ text: 'a\nb' }, { text: 'aXb' }, { text: 'abc' }]);

        // Without dotAll, . doesn't match newline
        // Pattern a.b matches "aXb" but not "a\nb" (newline not matched by .)
        const docsNoDotall = await collection.find({ text: { $regex: 'a.b' } }).toArray();
  ...
  describe('JavaScript RegExp', () => {
    ...
    it('should match with RegExp object', async () => {
      ...
      await collection.insertMany([{ name: 'Alice' }, { name: 'ALICE' }, { name: 'Bob' }]);

      const docs = await collection.find({ name: new RegExp('^alice', 'i') }).toArray();
    ...
    it('should work combined with other conditions', async () => {
      ...
      const docs = await collection.find({ name: /^A/, age: { $gt: 20 } }).toArray();
  ...
  describe('Array Fields', () => {
    it('should match if any array element matches', async () => {
      ...
      const docs = await collection.find({ tags: { $regex: '^prod' } }).toArray();
    ...
    it('should not match if no element matches', async () => {
      ...
      await collection.insertMany([{ tags: ['alpha', 'beta'] }, { tags: ['gamma', 'delta'] }]);
    ...
    it('should skip non-string elements in array', async () => {
      ...
      const docs = await collection.find({ values: { $regex: 'test' } }).toArray();
  ...
  describe('With $elemMatch', () => {
    ...
    it('should combine $regex with other conditions in $elemMatch', async () => {
      ...
      await collection.insertMany([
        ...
        {
          logs: [{ message: 'Fatal error', level: 'FATAL' }],
        },
      ]);
  ...
  describe('With $in Operator', () => {
    ...
    it('should work with array fields and $in containing regex', async () => {
      ...
      const docs = await collection.find({ tags: { $in: [/^prod/, /^staging/] } }).toArray();
  ...
  describe('With $not Operator', () => {
    it('should negate regex match with $not and $regex', async () => {
      ...
      const docs = await collection.find({ name: { $not: { $regex: '^Admin' } } }).toArray();
    ...
    it('should match missing fields with $not $regex', async () => {
      ...
      await collection.insertMany([{ name: 'Alice' }, { other: 'value' }, { name: 'test_user' }]);

      const docs = await collection.find({ name: { $not: { $regex: 'test' } } }).toArray();
  ...
  describe('With $nin Operator', () => {
    it('should not match if any RegExp in $nin matches', async () => {
      ...
      const docs = await collection.find({ status: { $nin: [/^active/, /^pending/] } }).toArray();
    ...
    it('should match if no RegExp in $nin matches', async () => {
      ...
      const docs = await collection.find({ category: { $nin: [/^electronics/] } }).toArray();
  ...
  describe('In Aggregation $match', () => {
    it('should support $regex in $match stage', async () => {
      ...
      const docs = await collection.aggregate([{ $match: { name: { $regex: '^A' } } }]).toArray();
    ...
    it('should support RegExp in $match stage', async () => {
      ...
      const docs = await collection.aggregate([{ $match: { email: /gmail\.com$/i } }]).toArray();
