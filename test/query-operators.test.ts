  ...
  describe("$type Operator", () => {
    describe("Basic Type Matching", () => {
      ...
      it("should match double type", async () => {
        ...
        // In JavaScript context, all numbers are doubles
        assert.strictEqual(docs.length, 2);
      });
      ...
      // Note: RegExp storage is only supported in MongoDB (BSON).
      // MangoDB uses JSON storage which cannot serialize RegExp objects.
      // This test only runs in MongoDB mode.
    });
